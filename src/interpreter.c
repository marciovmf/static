...
void symbol_table_set_variable_int(SymbolTable* table, const char* identifier, double value) 
{
  ...
  {
    if (strcmp(table->vars[i].identifier, identifier) == 0) 
    {
      table->vars[i].value = runtime_value_create_int(value);
      return;
    }
  }
  table->vars[table->count].identifier = strdup(identifier);
    ...
    case EXPR_BINARY:
      ...
      switch (expr->as.binary_expr.op) 
      {
        ...
        case OP_ASSIGN:
          break;
        default:
          log_and_break("Unknown binary operator");
          return runtime_value_create_void();
      }
    case EXPR_LVALUE:
      return symbol_table_get_variable(table, expr->as.identifier)->value;
    default:
    ...
    case AST_STATEMENT_ASSIGNMENT: 
      {
        ...
        {
          symbol_table_set_variable_int(table, stmt->as.assignment.identifier, value.as.number_value);
          return runtime_value_create_int(value.as.number_value);
        }
        else if (value.type == EXPR_LITERAL_INT)
        {
          symbol_table_set_variable_float(table, stmt->as.assignment.identifier, value.as.number_value);
          return runtime_value_create_float(value.as.number_value);
        }
        else if (value.type == EXPR_LITERAL_STRING)
        {
          symbol_table_set_variable_string(table, stmt->as.assignment.identifier, value.as.string_value);
...
int eval_program(SymbolTable* table, ASTProgram* program) 
{
  ...
  for (ASTStatementList* stmt_list = program->statements; stmt_list != NULL; stmt_list = stmt_list->next) 
  {
    last_value = eval_statement(table, stmt_list->statement);
